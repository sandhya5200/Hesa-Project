...
for sheet_name, dfs in combined_sheets.items():
    ...
    input_df['Company Address'] = input_df.apply(
        lambda row: normalized_mapping.get((row['Facilitator'].lower(), row['Customer State'].lower()), ("", ""))[0],
        axis=1
    )
    input_df['GSTIN'] = input_df.apply(
        lambda row: normalized_mapping.get((row['Facilitator'].lower(), row['Customer State'].lower()), ("", ""))[1],
        axis=1
    )
