...
sales_file_path = "/home/thrymr/Desktop/input Files(24-25)/original_sales(24-25)(6months agri,cons)/Consumer Sales - Jul 2024.xlsx"

# 
# /home/thrymr/Desktop/input Files(24-25)/original_sales(24-25)(6months agri,cons)/Consumer Sales - Jul 2024.xlsx
# /home/thrymr/Desktop/input Files(24-25)/original_sales(24-25)(6months agri,cons)/Consumer Sales - Aug 2024.xlsx
# /home/thrymr/Desktop/input Files(24-25)/original_sales(24-25)(6months agri,cons)/Consumer Sales - Sep 2024.xlsx

xls = pd.ExcelFile(sales_file_path)
dfs = []
for sheet in xls.sheet_names:
    print(f"reading sheet: {sheet}")
    df = pd.read_excel(sales_file_path, sheet_name=sheet)
    dfs.append(df)
sales_df = pd.concat(dfs, ignore_index=True)
print(f"concatenated all sheets and total rows are {len(sales_df)}")

purchases_df = pd.read_excel("/home/thrymr/Desktop/purchases(2024-25)/Hesa Consumer Products Private Limited/Consumer Purchase Jul-24.xlsx")

# 
# /home/thrymr/Desktop/purchases(2024-25)/Hesa Consumer Products Private Limited/Consumer Purchase Jul-24.xlsx
# /home/thrymr/Desktop/purchases(2024-25)/Hesa Consumer Products Private Limited/Consumer Purchase Aug-24.xlsx
# /home/thrymr/Desktop/purchases(2024-25)/Hesa Consumer Products Private Limited/Consumer Purchase Sep-24.xlsx
...
columns_to_extract = ['Date', 'Product Name', 'Product Qty', 'Customer District', 'Sub Vertical', 'Order Id', 'Customer State']
...
filtered_sales_df['State'] = filtered_sales_df['Customer State'].str.strip().str.replace(" ", "").str.lower()
purchases_df['Product Name'] = purchases_df['Product_Name'].str.strip().str.replace(" ", "").str.lower()
purchases_df['District'] = purchases_df['Vendor_District'].str.strip().str.replace(" ", "").str.lower()
purchases_df['State'] = purchases_df['Vendor_State'].str.strip().str.replace(" ", "").str.lower()
...
for idx, sales_row in filtered_sales_df.iterrows():
    ...
    if not purchase_rows.empty:
        for purchase_idx, purchase_row in purchase_rows.iterrows():
            total_purchase_qty += purchase_row['Product_Qty']
            po_numbers.append(purchase_row['Po_Number'])
    ...
    # Extra handling: 
    # Even after second-level matching, if total_purchase_qty is still less than required,
    # try to pick remaining purchases for same Product Name and State

    if total_purchase_qty < product_qty:
        remaining_rows = purchases_df[
            (purchases_df['Product Name'] == sales_row['Product Name']) &
            (purchases_df['State'] == sales_row['State'])
        ]

        for purchase_idx, purchase_row in remaining_rows.iterrows():
            total_purchase_qty += purchase_row['Product_Qty']
            po_numbers.append(purchase_row['Po_Number'])
            purchases_df.drop(index=purchase_idx, inplace=True)
    ...
    if total_purchase_qty > 0:  # Even if not fully matched, if some purchases are there
...
for (product, state), group in grouped:
    print(f"\nProcessing Product: {product}, State: {state}")
    ...
    for i, row in group.iterrows():
        print(f"\n Row {i}: Starting with Transfer Inwards = {row['Transfer Inwards']}")
        ...
        for j, transfer in enumerate(transfer_outwards):
            if transfer > 0 and remaining_excess > 0:
                ...
                print(f"  Deducted {deducted} from district '{districts[j]}'. Remaining Excess = {remaining_excess}")
        ...
        print(f" Updated row {i}: transfer_inwards_updated = {remaining_excess}, appended_districts = {appended_districts}")
...
print("\nFinished processing all groups and rows.")

output_file_path = "/home/thrymr/Downloads/inventory_report_Cons_July(24).xlsx"
print(f"Saving updated output with 'Transfer Outwards' and 'Closing Inventory' changes...")
print(f"Saving updated output to two sheets...")
# Save the unused purchases separately
unused_purchases_output_path = "/home/thrymr/Downloads/unused_purchases_july.xlsx"
purchases_df.to_excel(unused_purchases_output_path, index=False)

print(f"Unused purchases saved to {unused_purchases_output_path}")


split_point = 1048574

with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
    filtered_sales_df.iloc[:split_point].to_excel(writer, sheet_name='Sheet1', index=False)
    if len(filtered_sales_df) > split_point:
        filtered_sales_df.iloc[split_point:].to_excel(writer, sheet_name='Sheet2', index=False)

print("Updated output saved successfully to two sheets.")
