...
projections_df.rename(columns={"Sep'25": "projected_amount_cr"}, inplace=True) ####################---CHANGE---############################
...
merged_df['Date'] = merged_df['Date'].apply(lambda d: pd.Timestamp(year=2025, month=9, day=d.day))   #############---CHANGE---###############

merged_df["Cohort"] = "Sep'25"  #######################---CHANGE---#####################
...
agri_df.to_excel("/home/thrymr/Downloads/output_agri_sep.xlsx", index=False)

consumer_df = final_df[final_df['Vertical'] == 'Commerce Business']
consumer_df.to_excel("/home/thrymr/Downloads/output_cons_sep.xlsx", index=False)
